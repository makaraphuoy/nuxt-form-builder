...
function onStart(val: any) {
  emit("update:modelValue", { start: val ? String(val) : "", end: endStr.value });
}
function onEnd(val: any) {
  emit("update:modelValue", { start: startStr.value, end: val ? String(val) : "" });
}
</script>
...
<template>
  <div class="flex gap-3 items-end">
    ...
    <div class="flex-1">
      ...
      <UPopover>
        <UButton variant="outline" class="w-full justify-between" size="sm" type="button" :disabled="disabled">
          <span :class="startStr ? 'text-gray-900' : 'text-gray-400'">{{ startStr || "Select date…" }}</span>
    ...
    <UIcon name="i-heroicons-arrow-right" class="size-4 text-gray-400 shrink-0 mb-2" />
    ...
    <div class="flex-1">
      ...
      <UPopover>
        <UButton variant="outline" class="w-full justify-between" size="sm" type="button" :disabled="disabled">
          <span :class="endStr ? 'text-gray-900' : 'text-gray-400'">{{ endStr || "Select date…" }}</span>
          <UIcon name="i-heroicons-calendar" class="size-4 text-gray-400" />
        </UButton>
        <template #content>
